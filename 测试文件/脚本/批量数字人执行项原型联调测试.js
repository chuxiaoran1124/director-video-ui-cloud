...
async function main() {
  ...
  try {
    ...
    const humanScaleSlider = page.locator('[data-testid="human-scale-slider"]')
    const humanScaleValue = page.locator('[data-testid="human-scale-value"]')
    const humanScaleLock = page.locator('[data-testid="human-scale-lock"]')
    await humanScaleSlider.waitFor({ state: 'visible', timeout: 30000 })
    const initialHumanScale = await humanScaleValue.innerText()
    const sliderRunway = humanScaleSlider.locator('.el-slider__runway')
    const sliderBox = await sliderRunway.boundingBox()
    if (!sliderBox) throw new Error('数字人出镜大小滑块没有可操作的轨道')
    await sliderRunway.click({ position: { x: Math.floor(sliderBox.width * 0.7), y: Math.max(1, Math.floor(sliderBox.height / 2)) } })
    await page.waitForTimeout(100)
    const changedHumanScale = await humanScaleValue.innerText()
    if (changedHumanScale === initialHumanScale) throw new Error('数字人出镜大小滑块没有改变执行项配置')
    await humanScaleLock.click()
    if ((await humanScaleLock.innerText()).trim() !== '已锁定') throw new Error('数字人出镜大小没有进入锁定状态')
    const sliderDisabled = await humanScaleSlider.evaluate((element) => element.classList.contains('is-disabled') || element.getAttribute('aria-disabled') === 'true')
    if (!sliderDisabled) throw new Error('数字人出镜大小锁定后滑块仍可编辑')
    ...
    steps.push(`第1项数字人出镜大小从 ${initialHumanScale} 调整为 ${changedHumanScale}，锁定后保存草稿`)
    ...
    await humanScaleSlider.waitFor({ state: 'visible', timeout: 30000 })
    if ((await humanScaleValue.innerText()) !== changedHumanScale) throw new Error('重新打开计划后第1项数字人出镜大小没有恢复')
    if ((await humanScaleLock.innerText()).trim() !== '已锁定') throw new Error('重新打开计划后第1项数字人锁定状态没有恢复')
    steps.push('关闭并重新打开计划后，第1项数字人出镜大小和锁定状态均被恢复')
    ...
    await humanScaleSlider.waitFor({ state: 'visible', timeout: 30000 })
    if ((await humanScaleValue.innerText()) !== initialHumanScale) throw new Error('后续执行项错误地继承了第1项数字人大小')
    const secondSliderRunway = humanScaleSlider.locator('.el-slider__runway')
    const secondSliderBox = await secondSliderRunway.boundingBox()
    if (!secondSliderBox) throw new Error('后续执行项数字人大小滑块没有可操作的轨道')
    await secondSliderRunway.click({ position: { x: Math.floor(secondSliderBox.width * 0.3), y: Math.max(1, Math.floor(secondSliderBox.height / 2)) } })
    const secondHumanScale = await humanScaleValue.innerText()
    if (secondHumanScale === initialHumanScale) throw new Error('后续执行项数字人大小不能独立调整')
    steps.push('后续执行项声音只读，但数字人出镜大小仍可独立调整')
  ...
  fs.writeFileSync(MARKDOWN_PATH, [
    ...
    '## 覆盖操作', '',
    ...steps.map(step => `- ${step}`), '',
    error ? `## 失败原因\n\n${error}` : '## 结论\n\n添加空白执行项、清空、多选覆盖、声音跟随只读、单独选择无勾选框，以及每个执行项的数字人出镜大小独立调整、锁定和保存后恢复等操作均通过。'
  ].join('\n'), 'utf8')
