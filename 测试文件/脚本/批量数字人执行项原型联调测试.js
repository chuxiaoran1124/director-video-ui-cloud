...
async function selectFirstVoice(page) {
  ...
  const scaleSlider = humanDialog.getByTestId('human-picker-scale-slider')
  const scaleValue = humanDialog.getByTestId('human-picker-scale-value')
  const scaleLock = humanDialog.getByTestId('human-picker-scale-lock')
  await scaleSlider.waitFor({ state: 'visible', timeout: 30000 })
  if ((await scaleLock.innerText()).trim() === '已锁定') await scaleLock.click()
  const initialScale = await scaleValue.innerText()
  const runway = scaleSlider.locator('.el-slider__runway')
  const box = await runway.boundingBox()
  if (!box) throw new Error('数字人图片展示大小滑块没有可操作的轨道')
  ...
  if (!sliderDisabled) throw new Error('数字人图片展示大小锁定后滑块仍可编辑')
...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '添加', exact: true }).click()
    const scaleState = await selectFirstVoice(page)
    ...
    const reopenedHumanInput = page.locator('.performer-editor .picker-row input[placeholder*="可多选数字人"]').first()
    await reopenedHumanInput.click()
    const reopenedHumanDialog = page.locator('.el-dialog').filter({ hasText: '选择数字人形象' }).last()
    const reopenedScaleValue = reopenedHumanDialog.getByTestId('human-picker-scale-value')
    const reopenedScaleLock = reopenedHumanDialog.getByTestId('human-picker-scale-lock')
    ...
    steps.push('后续执行项声音只读，未出现可单独编辑的声音输入')
    ...
    if (!(await singleHumanDialog.getByTestId('human-picker-scale-slider').isVisible())) throw new Error('单独选择数字人模式缺少图片展示大小滑块')
